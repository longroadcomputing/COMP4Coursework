...
class RadioButtonWidget(QDialog):
    ...
    #construcor
    def __init__(self,label,instruction,button_list, parent):
        super().__init__()
        ...
        #connections
        self.select_button.clicked.connect(self.switchMainWindowViewView)
        self.cancel_button.clicked.connect(self.close)
        self.cancel_button.setShortcut("Ctrl+W")

    def switchMainWindowViewView(self):

        self.selectedTableOption =  self.radio_button_group.checkedId()

        if self.selectedTableOption == 1:
            self.parent.switchToNewItem()
        elif self.selectedTableOption == 2:
            self.parent.switchToNewCustomer()
        elif self.selectedTableOption == 3:
            self.parent.switchToNewLoan()
        elif self.selectedTableOption == 4:
            self.parent.switchToNewPatTest()
